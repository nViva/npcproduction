...
<div>
      ...
      <?php 
            ...
            if(($row['clients_number']*$row['price'])<=$row['amount_paid']&& $row['debit']!=0){
              ...
              $debt=$row['debit']+($row['clients_number']*$row['price']-$row['amount_paid']);
               ...
               $debt=0;
                   ...
                   <tr class="database-data" style="background-color: #2196f3;">
                    <th ><?php echo "<p style='text-align:center;color:black;'>".$count ."</p>"?></th>
                      ...
                      <th><?php echo "<p style='text-align:center;color:black;'>".number_format($debt)  ."</p>"?></th>
...
$sql = "SELECT employees.id,employees.emp_name,employees.job_name,employees.location,employees.emp_id,salaries.month,salaries.salary_amount,salaries.bonuses FROM employees JOIN salaries ON employees.id=salaries.emp_id WHERE employees.location='Berbershop' AND salaries.month='$month'";
                 ...
                 while ($row = $result4->fetch_assoc()) 
         {
            ...
            $salaries=$summ1+$summ2;
                   ?>
                <tr style='font-size:24px;font-weight: bold;color: black;background-color: #979A99;'><th colspan="5">Total </th><th><?php echo number_format($sum); ?></th><th><?php echo number_format($sum2); ?></th><th><?php echo number_format($sum3); ?></th><th><?php echo number_format($sum4); ?></th></tr>
<tr style='font-size:24px;font-weight: bold;color: black;'><th colspan="5">Salaries and Bonuses</th><th><?php echo number_format($salaries); ?></th><th></th><th></th><th></th></tr>
<tr style='font-size:24px;font-weight: bold;color: black;'><th colspan="5">Expenses</th><th><?php echo number_format($exp); ?></th><th></th><th></th><th></th></tr>

<tr style='font-size:24px;font-weight: bold;color: black;'><th colspan="5">Net Profit</th><th><?php echo number_format($sum-$salaries-$exp); ?></th><th></th><th></th><th></th></tr>
                 <tr><th>
