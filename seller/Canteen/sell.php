...
$result = mysqli_query($db, "SELECT * FROM products WHERE productId='" . $_POST["product_id"][$i] . "' AND location='Canteen' ");

$row[$i]= mysqli_fetch_array($result);
            
if ($row[$i]["balance"]>=$_POST["quant"][$i]) {

$selling=mysqli_query($db, "UPDATE products set balance=balance-'" . $_POST["quant"][$i] . "' WHERE productId='" . $_POST["product_id"][$i] . "' AND  " .$_POST["quant"][$i]."!=0 AND location='Canteen' ");
$sd=1;
if($sd)
{
